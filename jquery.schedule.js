;(function ($, window, document, undefined) {
    ...
    var defaults = {
            ...
            periodOptions: true,
            periodColors: [],
            limits:[8,20], // from 0 to 24
            periodTitle: "",
            periodBackgroundColor: "rgba(48, 198, 203, 0.5)",
            periodBorderColor: "#2a3cff",
            periodTextColor: "#000",
            periodRemoveButton: "Remove",
            periodDuplicateButton: "Duplicate",
            rowDuplicateButton: "Duplicate",
    ...
    $.extend(Plugin.prototype, {
        ...
        init: function () {
            ...
            if(this.settings.restricted_limits.length == 0){
                this.settings.restricted_limits = defaults.restricted_limits;
            }
            ...
            if (this.settings.mode === "edit") {
                ...
                $(this.element).on("mousedown", ".jqs-day", function (event) {
                    ...
                    if (!$(event.target).hasClass("jqs-period") && $(event.target).parents(".jqs-period").length === 0) {
                        ...
                        if ($this.settings.periodDuration !== 15) {
                            var limit_position = $this.settings.limits[0] * $this.periodInterval;
                            time = $this.periodInit(position + limit_position, position + limit_position + 1);
                        }
                ...
                $(this.element).on("mousemove", ".jqs-day", function (event) {
                    if (helper) {
                        ...
                        if (height > 1) {
                            var limit_position = $this.settings.limits[0] * $this.periodInterval;
                            $(".jqs-period-helper-time", helper).text($this.periodInit(position + limit_position, position + limit_position + height));
                        } else {
                ...
                //duplicate period
                $this = this;
                $(this.element).on("click", ".jqs-period .jqs-period-duplicate", function () {
                    var period = $(this).parents(".jqs-period");
                    for (var i = 0; i <= 6; i++) {
                        var parent = $(".jqs-day", $this.element).eq(i);
                        var options = {};
                        var height, position;
                        height = period.height() / $this.periodPosition;
                        position = period.position().top / $this.periodPosition;

                        position = position + $this.settings.limits[0] * $this.periodInterval;
                ...
                // delete a period
                $(this.element).on("click", ".jqs-period .jqs-period-remove", function () {
                    var period = $(this).parents(".jqs-period");
                ...
                // duplicate row
                $(this.element).on("click", ".jqs-grid-day .jqs-period-duplicate", function () {
                    var parent = $(".jqs-day", $this.element).eq(this.getAttribute("value"));
                    var dataDelete = parent[0].childNodes;

                    for (var i = dataDelete.length - 1; i >= 0; i--) {
                        var period = dataDelete[i];
                        for (var j = 0; j <= 6; j++) {
                            parent = $(".jqs-day", $this.element).eq(j);
                            var options = {};
                            var height, position;
                            height = parseInt(period.style.height) / $this.periodPosition;
                            position = parseInt(period.style.top) / $this.periodPosition;
                            position = position + $this.settings.limits[0] * $this.periodInterval;
                            $this.add(parent, position, height, options);
                        }
                    }
                ...
                // delete row
                $(this.element).on("click", ".jqs-grid-day .jqs-period-remove", function () {
                    var parent = $(".jqs-day", $this.element).eq(this.getAttribute("value"));
                    var dataDelete = parent[0].childNodes;

                    for (var i = dataDelete.length - 1; i >= 0; i--) {
                        if (!$this.settings.onRemovePeriod.call(this, dataDelete[i], $this.element)) {
                            dataDelete[i].remove();
                        }
                    }
                });
        ...
        create: function () {
            ...
            for (var i = 0; i < 7; i++) {
                var jqs_day_height = (this.settings.limits[1] - this.settings.limits[0])*40;
                $("<td><div class='jqs-day' style='height:"+jqs_day_height+"px'></div></td>").appendTo($(".jqs-table tr", this.element));
            }

            $("<div class='jqs-grid'><div class='jqs-grid-head'></div></div>").appendTo($(this.element));

            for (var j = this.settings.limits[0]; j <= this.settings.limits[1]; j++) {
            ...
            for (var k = 0; k < 7; k++) {
                ...
                var rowRemove = "";
                if (this.settings.mode === "edit") {
                    rowDuplicate = "<div class='jqs-period-duplicate' value='"+k+"' title='" + this.settings.rowDuplicateButton + "'></div>"
                    rowRemove = "<div class='jqs-period-remove' value='"+k+"' title='" + this.settings.rowRemoveButton + "'></div>";
                }
                $("<div class='jqs-grid-day'>" + this.settings.days[k] + "<br>" + rowDuplicate + rowRemove + "</div>").appendTo($(".jqs-grid-head", this.element));
            }
        },
        ...
        add: function (parent, position, height, options) {
            ...
            limit_height = this.settings.limits[0] * this.periodInterval;
            if(options!=undefined){
                position = position - limit_height;
            }
            ...
            var temp_position = position + limit_height;
            var periodTime = "<div class='jqs-period-time'>" + this.periodInit(temp_position, temp_position + height) + "</div>";
            var period = $("<div class='jqs-period'>" +
                "<div class='jqs-period-container'>" + periodTime + periodTitle + periodDuplicate + periodRemove + "</div>" +
                "</div>").css({
                    "top": position * this.periodPosition,
                    "height": height * this.periodPosition
                })
                ...
                period.draggable({
                    ...
                    stop: function (event, ui) {
                        if (!$this.isValid($(ui.helper))) {
                            ...
                            var start_pos = ui.originalPosition.top / $this.periodPosition;
                            var end_pos = start_pos + ui.helper["0"].clientHeight / $this.periodPosition;
                            ui.helper[0].getElementsByClassName("jqs-period-time")[0].innerText = $this.periodInit(start_pos, end_pos);
                        }
                    }
                }).resizable({
                    grid: [0, this.periodPosition],
                    ...
                    stop: function (event, ui) {
                        if (!$this.isValid($(ui.helper))) {
                            ...
                            var start_pos = ui.originalPosition.top / $this.periodPosition;
                            var end_pos = start_pos + ui.originalSize.height / $this.periodPosition;
                            ui.element[0].getElementsByClassName("jqs-period-time")[0].innerText = $this.periodInit(start_pos, end_pos);
                        }
                    }
                });
        ...
        periodDrag: function (ui) {
            var limit_position = this.settings.limits[0] * this.periodInterval;
            var start = Math.round(ui.position.top / this.periodPosition + limit_position);
            var end = Math.round(($(ui.helper).height() + ui.position.top) / this.periodPosition + limit_position);
        ...
        periodResize: function (ui) {
            var limit_position = this.settings.limits[0] * this.periodInterval;
            var start = Math.round(ui.position.top / this.periodPosition + limit_position);
            var end = Math.round((ui.size.height + ui.position.top) / this.periodPosition + limit_position);
        ...
        periodData: function (period) {
            var limit_position = this.settings.limits[0] * this.periodInterval;
            var start = Math.round(period.position().top / this.periodPosition + limit_position);
            var end = Math.round((period.height() + period.position().top) / this.periodPosition + limit_position);
        ...
        alsoPeriodData: function (period) {
            var limit_position = this.settings.limits[0] * this.periodInterval;
            var start = Math.round(period.position().top / this.periodPosition + limit_position);
            var end = Math.round((period.height() + period.position().top) / this.periodPosition + limit_position);
        ...
        isValid: function (current) {
            // console.log(this.settings.restricted_limits[0]);
            // console.log(current);
            ...
            var restricted_limits_check = false;
            ...
            $(".jqs-day", $this.element).each(function (index, day) {
                if($this.settings.restricted_limits[index]){
                    $(".jqs-period", day).each(function (day_index, period) {
                        if (current.attr("id") === $(period).attr("id")) {
                            for (var i = 0; i <= $this.settings.restricted_limits[index].periods.length-1; i++) {
                                var position = [];
                                for (var j = 0; j <= $this.settings.restricted_limits[index].periods[i].length-1; j++) {
                                    var split = $this.settings.restricted_limits[index].periods[i][j].split(":");
                                    var hour = parseInt(split[0]);
                                    var mn = parseInt(split[1]);
                                    position[j] = 0;
                                    position[j] += hour * $this.periodInterval;
                                    position[j] += mn / 60 * $this.periodInterval;
                                }
                                start = position[0];
                                end = position[1];

                                if (currentStart/ $this.periodPosition > start && currentStart/ $this.periodPosition < end) {
                                    restricted_limits_check = true;
                                }
                                if (currentEnd/ $this.periodPosition > start && currentEnd/ $this.periodPosition < end) {
                                    restricted_limits_check = true;
                                }
                                if (currentStart/ $this.periodPosition == start || currentEnd/ $this.periodPosition == end) {
                                    restricted_limits_check = true;
                                }
                                if (currentStart/ $this.periodPosition < start && currentEnd/ $this.periodPosition > end) {
                                    restricted_limits_check = true;
            ...
            if(restricted_limits_check==true){
                check = false;
            }
        ...
        exportAlsoData: function () {
            ...
            $(".jqs-day", $this.element).each(function (index, day) {
                var periods = [];
                $(".jqs-period", day).each(function (index, period) {
                    periods.push($this.alsoPeriodData($(period)));
                });
        ...
        import: function (args) {
            ...
            $.each(dataImport, function (index, data) {
                $.each(data.periods, function (index, period) {
                    ...
                    var min_position = $this.settings.limits[0] * $this.periodInterval;
                    var max_position = $this.settings.limits[1] * $this.periodInterval;
                    ...
                    if(min_position<=height || max_position>=position){
                        if(min_position>=position){
                            position = min_position;
                        }
                        if(max_position<=height){
                            height = max_position;
                        }
                        var status = true;
